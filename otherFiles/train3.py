    ...
    for train_idx, test_idx in kf.split(samples[:, 2]):  # 返回训练集和测试集的索引train：test 4:1
        ...
        for epoch in range(epochs):
            ...
            with torch.autograd.set_detect_anomaly(True):
                optimizer.zero_grad()
                score_train = model(src_train, dst_train, False)  # train集子图进入model训练
            ...
            with torch.no_grad():  # with torch.no_grad()或者@torch.no_grad()中的数据不需要计算梯度，也不会进行反向传播
                score_val = model(src_test, dst_test, True)  # 注意在整个图g中训练测试集
        ...
        with torch.no_grad():
            model.load_state_dict(best_model_state)
            score_test = model(src_test, dst_test, True)  # 测试分数和验证分数相同？？？
        ...
        print(f"Fold completed. Best validation AUC: %.4f' % {best_val_auc}")
