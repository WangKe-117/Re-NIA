...
class DeProp_method(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, dropout):
        ...
        for i in range(self.num_layers):
            self.convs.append(
                DeProp_Prop(hidden_channels, hidden_channels, self.lambda1, self.lambda2, self.gamma, self.orth))
            if self.with_bn:
                self.bns.append(torch.nn.BatchNorm1d(hidden_channels))

        self.conv_out = DeProp_Prop(hidden_channels, out_channels, self.lambda1, self.lambda2, self.gamma, self.orth)
